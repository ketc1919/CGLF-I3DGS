...
def build_voted_anchor_positions(scene, gaussians, pipe, background, depth_dir: Path, voxel_size: float,
...
def build_stage2_optimizer(gaussians: GaussianModel, lr_sr: float, lr_decoder: float):
    params = [
        {"params": [gaussians._anchor_feat_sr_mu], "lr": lr_sr},
        {"params": [gaussians._anchor_feat_sr_logvar], "lr": lr_sr},
    ]
...
def freeze_for_stage2(gaussians: GaussianModel):
    ...
    if gaussians.embedding_appearance is not None:
        for p in gaussians.embedding_appearance.parameters():
            p.requires_grad_(False)
    if gaussians.use_latent_field and gaussians.latent_field is not None:
        for p in gaussians.latent_field.parameters():
            p.requires_grad_(False)
...
def evaluate(scene, gaussians, pipe, background, out_dir: Path, max_views=8):
    ensure_dir(out_dir)
...
def build_renderable_stage2_state(gaussians: GaussianModel):
    state = {
...
def main():
    parser = ArgumentParser(description="Strict SuperGS-style stage-2")
    ...
    freeze_for_stage2(gaussians)
    optimizer = build_stage2_optimizer(gaussians, args.stage2_feature_lr, args.decoder_lr)
    lpips_fn = build_lpips() if build_lpips is not None else None

    train_cameras = scene.getTrainCameras()
    progress = tqdm(range(1, args.iterations + 1), desc="SuperGS strict stage2")
    for iteration in progress:
        if iteration <= args.densify_until and iteration % args.growth_interval == 1:
            new_xyz = build_voted_anchor_positions(
                scene, gaussians, pipe, background, depth_dir, args.vote_voxel_size,
        ...
        visible = prefilter_voxel(cam, gaussians, pipe, background)
        pkg = render(cam, gaussians, pipe, background, visible_mask=visible, retain_grad=False)
            ...
            torch.save(
                {
                    ...
                    "renderable_state": build_renderable_stage2_state(gaussians),
        ...
        if iteration % args.eval_interval == 0 or iteration == args.iterations:
            metrics = evaluate(scene, gaussians, pipe, background, stage2_output / f"eval_{iteration}", max_views=8)
