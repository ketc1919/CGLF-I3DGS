...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], ply_path=None):
        ...
        all_cameras = list(scene_info.train_cameras) + list(scene_info.test_cameras)
        if len(all_cameras) > 0:
            num_cameras = max(int(cam.uid) for cam in all_cameras) + 1
        else:
            num_cameras = 0
        self.gaussians.set_appearance(num_cameras)
