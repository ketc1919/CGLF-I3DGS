...
from dataloaders.read_write_model import read_model_poses, qvec2rotmat
...
class ImageDataset:
    ...
    def __init__(self, args: Namespace):
        ...
        self.global_mask_path = None
        if self.mask_dir and os.path.isfile(self.mask_dir):
            self.global_mask_path = self.mask_dir
            self.mask_dir = None
        if self.mask_dir:
            self.mask_paths = [
        ...
        for i, name in enumerate(self.infos):
            self.infos[name]["next_is_test"] = (args.test_hold > 0) and ((i + 1) % args.test_hold == 0)

        if args.shuffle:
            random.seed(0)
            lists = [self.image_name_list, self.image_paths]
            if self.mask_dir:
                lists.append(self.mask_paths)
            combined = list(zip(*lists))
            random.shuffle(combined)
            unpacked = [list(x) for x in zip(*combined)]
            self.image_name_list, self.image_paths = unpacked[0], unpacked[1]
            if self.mask_dir:
                self.mask_paths = unpacked[2]

        first_image = self._load_image(self.image_paths[0])
        self.width, self.height = first_image.shape[2], first_image.shape[1]
        res = self.width * self.height
        max_res = 2_000_000  # 2 Mpx
        ...
        if self.global_mask_path is not None:
            mask = cv2.imread(self.global_mask_path, cv2.IMREAD_GRAYSCALE)
            if mask is None:
                raise FileNotFoundError(f"Mask at {self.global_mask_path} could not be loaded.")
            if mask.shape != (self.height, self.width):
                mask = cv2.resize(mask, (self.width, self.height), interpolation=cv2.INTER_AREA)
            self.global_mask = (torch.from_numpy(mask).float() / 255.0)[None][None]

        # Load COLMAP pose metadata when present without parsing tracks/points.
    ...
    def __getitem__(self, index):
        ...
        if image.shape[0] == 4:
            info["mask"] = image[-1][None][None].cpu()  # [1, 1, H, W]
            image = image[:3]
        if self.mask_dir:
            mask = self._load_image(self.mask_paths[index])
            info["mask"] = mask[0][None][None]  # [1, 1, H, W]
        elif self.global_mask_path is not None:
            info["mask"] = self.global_mask

        return image.cuda()[None], info
    ...
    def load_colmap_data(self, colmap_folder_path):
        ...
        try:
            cameras, images = read_model_poses(colmap_folder_path)
    ...
    def align_colmap_poses(self):
        ...
        centres = []
        for idx in range(min(6, len(self.image_name_list))):
