...
public:
    ...
    __device__ Matx33f inv() const {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (pivotRow != i) {
                for (int j = 0; j < 3; ++j) {
                    swap(result(i, j), result(pivotRow, j));
                    swap(identity(i, j), identity(pivotRow, j));
                }
            }
...
{
    float n = norm(a);
    return n > 0 ? a / n : float3{0.f, 0.f, 0.f};
}
...
inline __device__ unsigned int xorshift32(unsigned int &state) {
...
// Picks `m` distinct indices in [0, N-1] based on seed `s`
// out must be preallocated with length >= m
inline __device__ void pickDistinctIndices(int* out, int m, int N, unsigned int seed) {
    if (m > N) { // impossible
        for (int i = 1; i < m; ++i) {
            out[i] = 0;
        }
    ...
    for (int i = 1; i < m; ++i) {
        ...
        for (int tries = 0; tries < 5; ++tries) {
            ...
            conflict = false;
            for (int j = 0; j < i; ++j) {
                if (idx == out[j]) { conflict = true; break; }
            }
            if (!conflict) break; // found unique
        }
        out[i] = idx; // may still collide, but very unlikely if N >> m
    }
}
